# 🔩 80 – Rahmen, Zugführungen, Kleinteile

**Baugruppe:** Rahmen · Ausfallende · Schaltauge · Zuganschläge · Ösen · Schrauben · Distanzhülsen ·
Federringe · Unterlegscheiben
**Zustand:** ✅ Rahmen okay · ⚠️ Kleinmaterial beim Umbau teils verloren (siehe Federn)

> ✅ **GEKLÄRT (2026-09-04/05):** Rahmen = **STAIGER Daytona Sportline**, **Alu 6061 Double
> Butted**, Rahmennummer **AWO7230329**, 28″/700C, **Gewindesteuersatz 1⅛″**, BSA-Innenlager.
> 🔴 **Neuer Messwert:** das Steuerrohr ist **20–30 mm länger** als das des Spenderrads
> (**Bergamont Horizon 4.0**) → Kern von Baustelle #1 und #6.
> 🔴 **Neuer Befund:** Bremssockel-Stift **M8** (Ø 7,95 mm gemessen) mit **M6-Innengewinde** –
> die Stifte der Bergamont-Gabel sind **zu kurz** → Baustelle #5.

---

## 1. Begriffe

| Deutsch | Englisch | Bei mir |
|---|---|---|
| Rahmen | frame | weiß |
| Ausfallende | dropout | ❓ horizontal / vertikal |
| **Schaltauge** | **derailleur hanger** | ❓ Zustand prüfen |
| Zuganschlag | cable stop | |
| Zugführung | cable guide | |
| Gewindeöse / Öse | eyelet / braze-on | |
| Innenlagergehäuse | BB shell | |
| Steuerrohr | head tube | |
| Distanzhülse | spacer / sleeve | |
| Wellenscheibe | wave washer | ⚠️ siehe Laufrad-Datei |
| Federring / Zahnscheibe | spring washer / toothed washer | |
| Unterlegscheibe | washer | |
| Sicherungsring / Sprengring | circlip / snap ring | |
| Schraubensicherung | threadlocker | ✅ Loctite blau 243 verwendet |
| Montagepaste | assembly compound / anti-seize | |

---

## 2. Rahmen

| Feld | Wert |
|---|---|
| Material | ✅ **Aluminium 6061, Double Butted** (Prägung `Alloy 6061, Double Butted`) – unmagnetisch |
| Bauart | ✅ **geschweißt** (Alu-Rahmen, Hydroforming-Ära) |
| Hersteller / Modell | ✅ **STAIGER Daytona Sportline** |
| Rahmenhöhe | ❓ ____ mm |
| Radstand | ❓ ____ mm |
| Kettenstrebenlänge | ❓ ____ mm (🔴 **wichtig für die Kettenlängen-Rechnung**, Baustelle #7) |
| 🔴 **Steuerrohrlänge** | ❓ ____ mm – **messen!** 🔴 **bereits bekannt: 20–30 mm länger als das Bergamont-Steuerrohr** |
| Steuerrohr-Ø innen (Lagerschalen) | 💡 bei 1⅛″: **EC34/28,6 – EC34/30** |
| Tretlagergehäusebreite | ✅ **68 mm** (BSA-Standard, passend zum **BB-ES25**) |
| Tretlagergewinde | ✅ **BSA 1,37" × 24 tpi** (links = Linksgewinde) |
| Zustand (Rost, Dellen, Risse) | ❓ 🔴 **Riss-Prüfung am Steuerrohr** (wurde mit fremder Gabel + Loctite-Provisorium gefahren) |
| Lack / Aufkleber | weiß |
| Rahmennummer | ✅ **AWO7230329** (Baujahr-Hypothese **2007, KW 23**) |

### 🔴 Riss-Prüfung (wichtig nach einem Umbau mit Press-/Ziehkräften)

An diesen Stellen mit einer hellen Lampe und ggf. Lupe prüfen:

| Stelle | Warum kritisch |
|---|---|
| Steuerrohr oben/unten (Übergang zum Ober-/Unterrohr) | Steuersatz wurde bearbeitet, Kräfte wirken hier |
| Tretlagergehäuse | Innenlager aus-/eingebaut |
| Ausfallende | Rad- und Schaltwerkskräfte |
| Schaltauge | Überstreckung durch zu kurze Kette möglich |
| Anlötteile / Ösen | Schutzblech- und Ständermontage |
| Sitzrohr an der Klemmung | Sattelstütze |

Bei **Alu**: Haarrisse sind schwer zu sehen. Verdacht → Werkstatt / Farbeindringprüfung.
Bei **Stahl**: Roststellen prüfen, ob es Oberflächen- oder Durchrostung ist.

---

## 3. Schaltauge

| Feld | Wert |
|---|---|
| Vorhanden? | ❓ ja / **angeformt** (bei Stahlrahmen oft Teil des Ausfallendes) / **angeschraubt** |
| Material | ❓ Alu (meist) |
| 🔴 **Zustand – gerade?** | 🔴 **UNGEPRÜFT UND VERDÄCHTIG** → **Baustelle #15**. Sicht von hinten: Schaltwerkskäfig muss **parallel** zu den Ritzeln stehen |
| Gewinde M10 × 1,0 für Schaltwerk | ❓ intakt? |
| Ersatz verfügbar? | ❓ 💰 **rahmenspezifisch**, 10–25 € |

> 🔴 **Warum das Schaltauge hier einen eigenen Verdacht hat (Baustelle #15):**
> Vor Jahren ein **Sturz** – *„hinten was verbogen"*, die **Schaltung ging nicht mehr
> richtig**. Der befreundete Händler hatte **das passende Teil nicht dabei** und baute
> **stattdessen vorne einen fremden Umwerfer (FD-C050)** ein.
> ⚠️ **„Hinten verbogen" + „Schaltung geht nicht richtig" ist die Lehrbuch-Signatur eines
> verbogenen Schaltauges** – behoben wurde aber **vorne**. 🔴 **Möglicherweise wurde die
> eigentliche Ursache nie angefasst.**
> → [`../06-logbuch/2026-09-04-vorgeschichte-haendlerfreund.md`](../06-logbuch/2026-09-04-vorgeschichte-haendlerfreund.md) Abs. 5

⚠️ **Ein krummes Schaltauge ist die häufigste Ursache für schlechte Schaltpräzision.**
Prüfung: von hinten auf das Rad schauen – Schaltwerkskäfig und Ritzel müssen **parallel** sein.
Richten mit einer **Schaltauge-Richtlehre** (💰 30–60 €) oder vorsichtig mit einem langen
Hebel. Bei Alu: **maximal 1–2× richten**, dann bricht es.

⚠️ **Zusammenhang mit deiner kurzen Kette:** Wenn die Kette in Big-Big das Schaltwerk
überstreckt, reißt es im Extremfall das Schaltauge ab. **Deshalb Big-Big vermeiden.**

---

## 4. Zugführungen & Anschläge

| Position | Typ | Zustand |
|---|---|---|
| Steuerrohr / Oberrohr (Schaltzug) | ❓ Anschlag / Führung | ❓ |
| Unterrohr (Umwerferzug) | ❓ | ❓ |
| Kettenstrebe (Schaltwerkzug) | ❓ außen geführt / innen? | ❓ |
| Oberrohr / Sitzstrebe (Bremszug hinten) | ❓ | ❓ |
| Tretlagerbereich (Lichtkabel) | ⚠️ **hier sind die Flachstecker reingerutscht** – jetzt gelöst | ✅ neu gelötet |
| Zugführung Schutzblech | ✅ integriert | ✅ |

**Prüfen:** Sind die Anschläge ausgebrochen? Rostig? Scharfkantig (scheuert die Hülle durch)?
→ Mit **Kabeldurchführungstüllen** (Grommets) nachrüsten, 💰 1–2 €.

---

## 5. Gewindetabelle Fahrrad (Nachschlagewerk)

| Anwendung | Gewinde | Hinweis |
|---|---|---|
| Achse hinten, Vollachse | M10 × 1,0 | Feingewinde |
| Achse vorne, Vollachse | M9 × 1,0 | |
| Schnellspanner | M5 (Stange) | |
| Freilaufkörper-Befestigung an der Nabe | M35 × 1,0 (Shimano) | großer Inbus/Schlüssel |
| Kassetten-Lockring | Sondergewinde | Werkzeug: 12-Spline-Abzieher |
| Pedale | 9/16" × 20 tpi | links = Linksgewinde! |
| Innenlager BSA | 1,37" × 24 tpi | links = Linksgewinde! |
| Innenlager italienisch | 36 mm × 24 tpi | **beide Seiten Rechtsgewinde** ⚠️ Verwechslungsgefahr |
| Steuersatz 1" | 25,4 mm × 24 tpi (1,058 mm) | ⚠️ **kein Metrik-Gewinde!** |
| Steuersatz 1⅛" | 28,6 mm × 24 tpi | ⚠️ ✅ **dein Fall** – **keine Baumarkt-Mutter!** |
| Vorbau-Keilschraube (Quill) | M6 oder M8 | Inbus |
| Vorbau-Faceplate | M5 | Inbus, 5–6 Nm |
| Lenkerklemmschrauben | M5 / M6 | 5–8 Nm |
| 🔴 **Bremssockel-Stift (Cantistift) am Rahmen/Gabel** | **M8 außen** (Ø gemessen **7,95 mm**), **M6 innen** | **5–6 Nm** + **Loctite 243** ins **Alu**-Tauchrohr, **2–3 h** warten |
| Bremskörper-Befestigung (Bremsarm → Stift) | M6 | 6–8 Nm |
| 🔴 **Bremshebel-Klemmschelle am Lenker** | M5 | **4–6 Nm** (Alu-Lenker!) – ⚠️ **zu fest/zu nah → Angelpunkt geklemmt** = war Baustelle #3 |
| Schutzblechschrauben | M5 | max. 5 Nm |
| Ständerschrauben | M5 / M6 | |
| Gepäckträgerschrauben | M5 | |
| Sattelklemmschraube | M5 / M6 / Schnellspanner | 5–8 Nm |
| Sattelstreben-Klemmung | M5 | 15–20 Nm (je nach Hersteller) |
| Kettenblattschrauben | M5 / M7 | 8–14 Nm |
| Schaltwerks-Befestigung | M10 × 1,0 | 8–10 Nm |
| Zugklemmschrauben | M4 / M5 | 5–7 Nm |
| Schalthebel-Klemmschelle | M5 | 5–7 Nm |
| Umwerferschelle | M5 | 5–7 Nm |
| Kurbelschraube (Vierkant) | M8 / M14 | 35–50 Nm |
| Schutzblech-Strebenschrauben | M4 / M5 | |

### Anzugsmomente (Richtwerte, Alu-Teile!)

| Verbindung | Nm |
|---|---|
| Vorbau-Faceplate (Lenker) | 5–6 |
| Quill-Vorbau Keilschraube | 20–25 |
| Ahead-Schaftklemmschrauben | 5–6 |
| Ahead-Topcap | nur **spielbeseitigend** (ca. 1–2 Nm) |
| Sattelklemme | 5–8 |
| Lenker-Klemmschrauben | 5–8 |
| Bremskörper-Schraube | 6–8 |
| Schutzblech M5 | ≤ 5 |
| Kassette-Lockring | **40–50** |
| Kurbelschraube Vierkant | 35–50 |
| Pedale | 35–40 |
| Schnellspanner | Handkraft (kein Drehmoment!) |
| Konus Nabe | Handkraft + 1/16 Umdrehung |

⚠️ **Ohne Drehmomentschlüssel:** „handfest plus eine Viertel Umdrehung“ bei Alu-Schrauben,
bei M5-Inbus mit einem kurzen Schlüssel nie mit voller Kraft.

---

## 6. Kleinteile-Inventar (was habe ich überhaupt?)

Beim Umbau gehen Kleinteile verloren. Diese Liste hilft, den Bestand zu erfassen:

| Teil | Anzahl vorhanden | Passt zu | Notiz |
|---|---|---|---|
| Distanzscheiben M5 | ❓ | Schutzblech, Ständer | |
| Distanzscheiben M6 | ❓ | Bremsen | |
| Distanzhülsen (Röhrchen) | ❓ | Achsen | |
| Federringe / Zahnscheiben | ❓ | Kontermuttern | |
| Wellenscheiben | ❓ | Industrielager | ⚠️ eine verloren |
| Schnellspanner-Federn | ❓ | QR | ⚠️ eine verloren? |
| Sprengringe | ❓ | Naben | |
| M5-Schrauben (verschiedene Längen) | ❓ | Anbauteile | |
| M6-Schrauben | ❓ | Bremsen | |
| Kontermuttern Steuersatz | ❓ | Gewindesteuersatz | ⚠️ fehlt/zu dick |
| Kugeln / Kugelkäfige | ❓ | Lager | |
| Zugendhülsen (5 mm / 4 mm) | ❓ | Züge | |
| Kabelschellen | ❓ | Licht | |
| Kabelbinder | ❓ | – | |

💡 **Empfehlung:** Eine **Sortimentsbox** (💰 10–20 €) mit M5/M6-Schrauben, Scheiben,
Muttern und Distanzhülsen kaufen. Damit lassen sich 90 % der Winkel-/Abstandsprobleme
(Schutzblech, Ständer, Träger) lösen, ohne jedes Mal zu bestellen.

---

## 7. 📐 Messliste

| # | Messung | Wert |
|---|---|---|
| 1 | Rahmenmaterial (Magnet-Test) | Stahl / Alu |
| 2 | Rahmennummer | |
| 3 | Tretlagergehäusebreite | ____ mm |
| 4 | Kettenstrebenlänge | ____ mm |
| 5 | Radstand | ____ mm |
| 6 | Ausfallende horizontal/vertikal + Einbaubreite | |
| 7 | 🔴 Schaltauge: vorhanden, **gerade**? (Baustelle **#15** – Sturz-Folge ungeprüft) | |
| 8 | Riss-Prüfung (Abschnitt 2) | |
| 9 | Zuganschläge: Zustand | |
| 10 | Ösen-Liste: wo sind Gewindeösen (Größe M5/M6)? | |

## 8. 💰 Einkaufsbedarf

| Teil | Spec | Prio | ca. Preis | bestellt? |
|---|---|---|---|---|
| Kleinteile-Sortiment | M5/M6 Schrauben, Scheiben, Muttern, Hülsen | 🟡 | 10–20 € | ☐ |
| 🔴 **Schaltauge** (falls krumm/gebrochen) | **rahmenspezifisch** – Hersteller + Modell nötig. 🔴 **Baustelle #15:** nach dem Sturz vor Jahren ungeprüft. 💡 **Alu-Schaltaugen lassen sich max. 1–2× richten**, dann tauschen | 🟡 | 10–25 € | ☐ |
| Kabeldurchführungstüllen | 5–8 mm | 🟢 | 2–5 € | ☐ |
| Drehmomentschlüssel | 2–25 Nm, mit Inbus-Bits | 🟢 | 30–60 € | ☐ |
